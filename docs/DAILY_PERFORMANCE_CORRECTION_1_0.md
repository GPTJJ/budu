...
Legacy draft revenue and staffing writes use the same store-day lock and recheck status/version inside their transaction. A request that checked a draft before an administrator's correction cannot later overwrite the confirmed correction. The isolated regression deliberately parks each old HTTP writer on that lock, commits the new correction, then requires the old request to reject with 409 and preserve both canonical tables.
