...
The separate `MONTHLY_UNIFIED_SUMMARY:<periodStart>:<periodEnd>` management report is a result aggregator, not another Payroll audit. It reuses the exact `MONTHLY_FULL_TIME` model and the intersecting `WEEKLY_PART_TIME` models, slices persisted daily payroll results at calendar-month boundaries, and records every source run ID. Missing sources return `SOURCE_REPORT_MISSING`; legacy weekly artifacts without verified daily component/final-pay breakdown return `MONTH_BOUNDARY_SOURCE_INSUFFICIENT`. The summary never invokes the Payroll resolver or rescans `DailyStoreStaff`.
