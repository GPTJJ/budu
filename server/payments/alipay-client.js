...
export class AlipayClientError extends Error {
  constructor(code, message, { ambiguous = true, status = 0 } = {}) {
    super(message)
...
export class AlipayOpenApiClient {
  ...
  async request(path, body, requestId) {
    ...
    try {
      ...
      throw new AlipayClientError('NETWORK_OR_PROVIDER_ERROR', '支付宝请求结果未知，需要主动查询', { ambiguous: status === 0 || status >= 500, status })
    } finally {
