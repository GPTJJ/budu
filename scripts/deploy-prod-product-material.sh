...
CANDIDATE="budu-prod-${SHORT_SHA}-product-material"
MIGRATOR="budu-migrate-${SHORT_SHA}-product-material"
BACKUP_CONTAINER="budu-backup-${SHORT_SHA}-product-material"
IMAGE="budu-api:product-material-${SHORT_SHA}"
HOST_TEMPLATE="${APP_DIR}/deploy/nginx/conf.d/budu.conf.template"
ACTIVE_CONFIG="/etc/nginx/conf.d/budu.conf"
ENV_FILE="${APP_DIR}/.env.production"
WORK_ROOT="$(mktemp -d "/dev/shm/budu-product-material-${SHORT_SHA}.XXXXXX")"
BINDING_FILE="${WORK_ROOT}/recipient-binding.json"
ROLLBACK_ROOT="${APP_DIR}/.rollback-assets/product-material-${SHORT_SHA}-$(date -u +%Y%m%dT%H%M%SZ)"
OLD_CONTAINER=""
OLD_STOPPED=0
TEMPLATE_CHANGED=0
ENV_CHANGED=0
...
console.log(JSON.stringify({ publicHealth: true, database: 'budu_bj006', migrations: 51 }))
NODE
}
...
echo "production authority verified: DB=budu_bj006 migration=50 health=PASS writer=1"
...
path.chmod(0o600)
PY
BACKUP_NAME="budu_bj006-migration50-pre-product-material-${SHORT_SHA}.dump"
...
echo "fresh migration50 backup integrity PASS; protected rollback copy created; product/material baselines locked"

# Run the exact release migrator in isolation. Migration 51 only adds transfer-specific additive or
# defaulted master/snapshot columns; all historical business facts must stay unchanged.
...
echo "migration ledger advanced 50→51; historical transfer, purchase, mailing and invoice facts unchanged"
...
verify_database_authority "$CANDIDATE" 51
...
DEPLOY_OK=1
echo "Product/material management additive blue/green deployment completed; historical facts unchanged"
