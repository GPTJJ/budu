...
test('18 presentation renderer contract is replaceable', () => { assert.equal(SWEET_CARD_PRESENTATION_CONTRACT.templateKey, 'minimal-v2'); assert.ok(SWEET_CARD_PRESENTATION_CONTRACT.slots.includes('designVersion')); assert.ok(SWEET_CARD_PRESENTATION_CONTRACT.authorityExclusions.includes('balance')) })
...
test('28 management surface has a reachable usage view wired to the scoped usage data path', () => { const ui = read('src/components/SweetCardPage.jsx'); assert.match(ui, /const openUsage = \(\) => setUsageOpen\(true\)/); assert.match(ui, /onClick=\{openUsage\}[\s\S]{0,300}使用记录<\/button>/); assert.match(ui, /usageOpen \? <section[\s\S]{0,1800}data\.usage\.map/); assert.match(ui, /api\(`\/v2\/sweet-cards\/usage\$\{scopeQuery\}`\)/) })
test('28a management surface retains filtering, presets and complete carrier details', () => { const ui = read('src/components/SweetCardPage.jsx'); const wizard = read('src/components/sweet-card/SweetCardCreateWizard.jsx'); assert.match(wizard, /¥500/); assert.match(wizard, /¥1000/); assert.match(ui, /按批次筛选/); assert.match(ui, /Credential/) })
...
test('30a Gate 1.3 surface: rules empty state, manage compact entry, step1 slim, step3 back button, overview dedup', () => { const ui = read('src/components/SweetCardPage.jsx'); const wizard = read('src/components/sweet-card/SweetCardCreateWizard.jsx'); assert.match(ui, /当前暂无商品分类/); assert.match(ui, /categories\.length === 0/); assert.match(ui, /hidden sm:inline">管理设置/); assert.match(wizard, /flex-wrap items-center gap-3/); assert.match(wizard, /whitespace-nowrap/); assert.match(wizard, /\/>上一步/) })
