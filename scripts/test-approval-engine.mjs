...
ok('财务不可见待审批', !canViewRequest(finance, reqPending, { template }))
ok('财务不可见已驳回', !canViewRequest(finance, reqRejected, { template }))
...
ok('财务不可归档', !canArchive(finance, reqApproved))
...
ok('all+finance 仅已通过', scopeFilter('all', finance).status === 'approved')
