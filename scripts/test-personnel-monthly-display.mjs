...
assert.match(personnelSource, /getDailyStoreStaffMonthState\(m\)/)
assert.match(personnelSource, /monthState\.status !== 'loaded' \|\| !monthState\.hasPayload/)
assert.match(personnelSource, /工资数据暂不可用/)
assert.match(personnelSource, /payrollAvailable: true/)
assert.match(personnelSource, /payrollAvailable: false/)
assert.match(personnelSource, /emp\.payrollAvailable \? \(/)
assert.doesNotMatch(personnelSource, /payrollDisplay\.byEmployeeId\.get\(d\.id\) \|\| \{\}/)
