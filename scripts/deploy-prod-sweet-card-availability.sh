...
RESTORE_CONTAINER="budu-restore-${SHORT_SHA}-sweet-card-availability"
ISOLATED_NETWORK="budu-isolated-${SHORT_SHA}-sweet-card-availability"
IMAGE="budu-api:sweet-card-availability-${SHORT_SHA}"
HOST_TEMPLATE="${APP_DIR}/deploy/nginx/conf.d/budu.conf.template"
ACTIVE_CONFIG="/etc/nginx/conf.d/budu.conf"
WORK_ROOT="$(mktemp -d "/dev/shm/budu-sweet-card-availability-${SHORT_SHA}.XXXXXX")"
ROLLBACK_ROOT="${APP_DIR}/.rollback-assets/sweet-card-availability-${SHORT_SHA}-$(date -u +%Y%m%dT%H%M%SZ)"
...
rollback_on_error() {
  ...
  echo "SWEET_CARD_AVAILABILITY_DEPLOY_ROLLBACK_START" >&2
...
before['counts'].pop('audits',None); after['counts'].pop('audits',None)
...
docker run -d --name "$RESTORE_CONTAINER" --network "$ISOLATED_NETWORK" \
  -e POSTGRES_USER=restore -e POSTGRES_PASSWORD=restore -e POSTGRES_DB=budu_sc_availability_isolated \
  -v "${ROLLBACK_ROOT}:/backup:ro" postgres:16-alpine >/dev/null
for _attempt in $(seq 1 30); do docker exec "$RESTORE_CONTAINER" pg_isready -U restore -d budu_sc_availability_isolated >/dev/null 2>&1 && break; sleep 1; done
docker exec "$RESTORE_CONTAINER" pg_isready -U restore -d budu_sc_availability_isolated >/dev/null
docker exec "$RESTORE_CONTAINER" pg_restore -U restore -d budu_sc_availability_isolated --no-owner "/backup/${PRE_BACKUP}"
ISOLATED_DATABASE_URL="postgresql://restore:restore@${RESTORE_CONTAINER}:5432/budu_sc_availability_isolated"
docker run --rm --network "$ISOLATED_NETWORK" -e DATABASE_URL="$ISOLATED_DATABASE_URL" "$IMAGE" npx prisma migrate deploy
ISOLATED_IDENTITY="$(docker exec "$RESTORE_CONTAINER" psql -U restore -d budu_sc_availability_isolated -Atc 'SELECT current_database() || '\''|'\'' || (SELECT count(*) FROM "_prisma_migrations" WHERE finished_at IS NOT NULL AND rolled_back_at IS NULL) || '\''|'\'' || (SELECT count(*) FROM "_prisma_migrations" WHERE started_at IS NOT NULL AND finished_at IS NULL AND rolled_back_at IS NULL) || '\''|'\'' || (SELECT count(*) FROM "sweet_card_batches" WHERE "archived_at" IS NOT NULL);')"
[ "$ISOLATED_IDENTITY" = "budu_sc_availability_isolated|67|0|0" ]
docker run --rm --network "$ISOLATED_NETWORK" \
  -e DATABASE_URL="$ISOLATED_DATABASE_URL" \
  -e JWT_SECRET=isolated-sweet-card-availabilityanization-secret \
  -e SWEET_CARD_ENABLED=1 -e XIDAN_SWEET_CARD_COMMERCIAL=1 \
  -e SWEET_CARD_CREDENTIAL_KEY="$(printf '11%.0s' $(seq 1 32))" \
  "$IMAGE" node scripts/test-sweet-card-store-availability.mjs
docker rm -f "$RESTORE_CONTAINER" >/dev/null
docker network rm "$ISOLATED_NETWORK" >/dev/null
echo "restored M66 to M67 and real API integration PASS"
...
verify_database "$CANDIDATE" 67
[ "$(count_writers "$OLD_CONTAINER")" -eq 1 ]
docker exec "$CANDIDATE" env EXPECTED_RELEASE_SHA="$RELEASE_SHA" node /app/scripts/verify-sweet-card-availability-production.mjs
...
PY
docker exec "$CANDIDATE" env EXPECTED_RELEASE_SHA="$RELEASE_SHA" node /app/scripts/verify-sweet-card-availability-production.mjs | tee "${ROLLBACK_ROOT}/production-verification.json"
chmod 400 "${ROLLBACK_ROOT}/production-verification.json"
docker exec "$CANDIDATE" node scripts/initialize-sweet-card-store-authority.mjs expand
docker exec "$CANDIDATE" node scripts/monitor-sweet-card-availability.mjs
FINAL_JSON="$(audit_container "$CANDIDATE")"
compare_business_facts "$BEFORE_JSON" "$FINAL_JSON"
printf '%s\n' "$FINAL_JSON" > "${ROLLBACK_ROOT}/production-after-m67.json"
chmod 400 "${ROLLBACK_ROOT}/production-after-m67.json"

POST_BACKUP="current-canonical-budu_bj006-m67-store-availability.dump"
create_backup "$POST_BACKUP"
docker run -d --name "$RESTORE_CONTAINER" -e POSTGRES_USER=restore -e POSTGRES_PASSWORD=restore -e POSTGRES_DB=budu_restore_m67 -v "${ROLLBACK_ROOT}:/backup:ro" postgres:16-alpine >/dev/null
for _attempt in $(seq 1 30); do docker exec "$RESTORE_CONTAINER" pg_isready -U restore -d budu_restore_m67 >/dev/null 2>&1 && break; sleep 1; done
docker exec "$RESTORE_CONTAINER" pg_isready -U restore -d budu_restore_m67 >/dev/null
docker exec "$RESTORE_CONTAINER" pg_restore -U restore -d budu_restore_m67 --no-owner "/backup/${POST_BACKUP}"
RESTORE_RESULT="$(docker exec "$RESTORE_CONTAINER" psql -U restore -d budu_restore_m67 -Atc 'SELECT current_database() || '\''|'\'' || (SELECT count(*) FROM "_prisma_migrations" WHERE finished_at IS NOT NULL AND rolled_back_at IS NULL) || '\''|'\'' || (SELECT count(*) FROM "_prisma_migrations" WHERE started_at IS NOT NULL AND finished_at IS NULL AND rolled_back_at IS NULL) || '\''|'\'' || (SELECT count(*) FROM "sweet_card_batches") || '\''|'\'' || (SELECT count(*) FROM "sweet_card_batches" WHERE "archived_at" IS NOT NULL);')"
[ "$RESTORE_RESULT" = "budu_restore_m67|67|0|${BEFORE_BATCH_COUNT}|0" ]
docker rm -f "$RESTORE_CONTAINER" >/dev/null
...
DEPLOY_OK=1
echo "SWEET_CARD_STORE_AVAILABILITY_1_0_COMPLETE sha=${RELEASE_SHA} runtime=${CANDIDATE} migration=67 backup=${ROLLBACK_ROOT}/${POST_BACKUP} backup_sha256=${BACKUP_SHA} restore_listing=${RESTORE_LISTING} writer=1"
