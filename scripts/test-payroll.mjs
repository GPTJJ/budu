...
for (const c of monthCases) {
  const ok = c.got && Object.entries(c.expect).every(([k, v]) => Math.abs(c.got[k] - v) < 1e-6)
  if (!ok) {
    failed += 1
    console.log('FAIL:', c.name, JSON.stringify(c.got), 'expect', JSON.stringify(c.expect))
  } else {
...
if (!normal || normal.salary !== 264) {
  failed += 1
  console.log('FAIL 同班次普通员工工资计算错误:', JSON.stringify(normal))
} else {
  console.log('OK 同班次普通员工工资正常')
}
