#!/usr/bin/env bash
# Authority-aware additive blue/green deployment for product/material management.
...
CANDIDATE="budu-prod-${SHORT_SHA}-product-category-summary"
MIGRATOR="budu-migrate-${SHORT_SHA}-product-category-summary"
BACKUP_CONTAINER="budu-backup-${SHORT_SHA}-product-category-summary"
IMAGE="budu-api:product-category-summary-${SHORT_SHA}"
HOST_TEMPLATE="${APP_DIR}/deploy/nginx/conf.d/budu.conf.template"
ACTIVE_CONFIG="/etc/nginx/conf.d/budu.conf"
ENV_FILE="${APP_DIR}/.env.production"
WORK_ROOT="$(mktemp -d "/dev/shm/budu-product-category-summary-${SHORT_SHA}.XXXXXX")"
BINDING_FILE="${WORK_ROOT}/recipient-binding.json"
ROLLBACK_ROOT="${APP_DIR}/.rollback-assets/product-category-summary-${SHORT_SHA}-$(date -u +%Y%m%dT%H%M%SZ)"
OLD_CONTAINER=""
OLD_STOPPED=0
TEMPLATE_CHANGED=0
ENV_CHANGED=0
...
try {
  const rows = await prisma.purchaseRequest.findMany({
    orderBy: { id: 'asc' },
    select: {
      id: true, storeKey: true, status: true, supplier: true, supplierId: true,
      expectedAt: true, note: true, createdBy: true, createdAt: true, updatedAt: true,
      items: {
        orderBy: { id: 'asc' },
        select: {
          id: true, itemId: true, orderedQty: true, receivedQty: true, note: true,
          item: { select: { name: true, category: true, sku: true, barcode: true } },
        },
      },
    },
...
try {
  const rows = await prisma.$queryRawUnsafe(`
    SELECT
      "category",
      COUNT(*)::int AS "total",
      COUNT(*) FILTER (WHERE "transferEnabled")::int AS "enabled",
      COUNT(*) FILTER (WHERE "transferEnabled" AND "transferCode" IS NOT NULL)::int AS "coded"
    FROM "InventoryItem"
...
verify_empty_product_category_authority() {
  local container="$1"
  docker exec -i "$container" node --input-type=module - <<'NODE'
import { PrismaClient } from '@prisma/client'
const prisma = new PrismaClient()
try {
  const rows = await prisma.$queryRawUnsafe(`
    SELECT
      (SELECT COUNT(*)::int FROM "ProductCategory") AS "categories",
      (SELECT COUNT(*)::int FROM "InventoryItem" WHERE "category" = 'product' AND "productCategoryId" IS NOT NULL) AS "classifiedProducts",
      (SELECT COUNT(*)::int FROM "TransferItem" WHERE "productCategoryNameSnapshot" <> '') AS "classifiedHistoricalItems"
  `)
  const result = {
    categories: Number(rows[0]?.categories || 0),
    classifiedProducts: Number(rows[0]?.classifiedProducts || 0),
    classifiedHistoricalItems: Number(rows[0]?.classifiedHistoricalItems || 0),
  }
  if (result.categories !== 0 || result.classifiedProducts !== 0 || result.classifiedHistoricalItems !== 0) {
    throw new Error('CATEGORY_MIGRATION_MUST_NOT_CLASSIFY_EXISTING_FACTS')
  }
  console.log(JSON.stringify(result))
} finally {
  await prisma.$disconnect()
...
console.log(JSON.stringify({ publicHealth: true, database: 'budu_bj006', migrations: 52 }))
NODE
}
...
verify_database_authority "$OLD_CONTAINER" 51
...
echo "production authority verified: DB=budu_bj006 migration=51 health=PASS writer=1"
...
path.chmod(0o600)
PY
BACKUP_NAME="budu_bj006-migration51-pre-product-category-summary-${SHORT_SHA}.dump"
...
echo "fresh migration51 backup integrity PASS; protected rollback copy created; product/material baselines locked"

# Run the exact release migrator in isolation. Migration 52 only adds the product-category authority and historical category snapshot
# columns; all existing product and historical business facts must stay unchanged.
...
verify_empty_product_category_authority "$OLD_CONTAINER"
echo "migration ledger advanced 51→52; historical transfer, purchase, mailing and invoice facts unchanged"
...
verify_database_authority "$CANDIDATE" 52
...
verify_empty_product_category_authority "$CANDIDATE"

printf '%s\n' "$RELEASE_SHA" > "${APP_DIR}/.current-sha"
DEPLOY_OK=1
echo "Product category and transfer summary additive blue/green deployment completed; existing products and historical facts unchanged"
