...
assert.match(personnelSource, /employeeDayStatus\(/)
assert.match(personnelSource, /employeeWeekStatus\(/)
assert.ok(personnelSource.split('stablePeriod ? emp.id : undefined').length - 1 >= 2)
console.log('  [J] 日期/周请求序号防迟到覆盖；当前 Employee.id 在渲染时绑定 PASS')
