...
const records = [
  {
    ...
    items: [
      ...
      { category: 'product', productCategory: '糖果', itemCode: 'NO.2', productName: '柠檬', quantity: null, boxQuantity: 1, pieceQuantity: 166, boxWeightGrams: 2500, pieceWeightGrams: 6 },
...
test('汇总只使用已发货与发货确认时间，并按门店拆分调入调出和净调拨', () => {
  ...
  assert.deepEqual([tongyingMixed.调入箱数, tongyingMixed.调入散颗数, tongyingMixed['净估算重量（约kg）']], [1, 166, 3.496])
})
...
test('Excel 同时包含调拨汇总与调拨明细并保留追溯字段', () => {
  ...
  assert.deepEqual(Object.keys(detailRows[0]), ['调拨单号', '发货时间', '调出门店', '调入门店', '类型', '产品分类', '编号', '名称', '历史数量（件）', '箱数', '散颗数', '估算重量（约kg）', '申请人', '发货确认人', '备注'])
})
