...
const authorityTables = ['orders', 'order_items', 'payments', 'refunds', 'refund_items', 'TransferRequest', 'TransferItem', 'InventoryItem', 'DailyEntry', 'daily_store_staff', 'payroll_notices', 'Expense']
const authorityDigest = async (client) => Object.fromEntries(await Promise.all(authorityTables.map(async (table) => [table, sha(await client.$queryRawUnsafe(`SELECT * FROM "${table}" ORDER BY "id"`))])))
...
test('production 58 → report 59/60 → cost 61/62 preserves historical sales facts', async () => {
  ...
  try {
    ...
    const authoritiesBefore = await authorityDigest(client)
    ...
    assert.equal(sha(await client.$queryRawUnsafe(factSql)), before)
    assert.deepEqual(await authorityDigest(client), authoritiesBefore)
    ...
    const exact = await authority.report(user, { from: '2026-08-01', to: '2026-08-31', compare: 'previous', period: 'month' })
    assert.equal(exact.state, 'EXACT')
    assert.equal(exact.totals.cogsCents, '900', '赠品计入快照 COGS，退款状态不会自动冲回')
    assert.equal(exact.stores[0].components.cogs.source, 'ORDER_ITEM_COST_PRICE_SNAPSHOT')
    assert.equal(exact.stores[0].components.labor.breakdown.wagesCents, '10000')
    assert.equal(exact.stores[0].components.labor.confirmedZero, false)
    assert.equal(exact.storeGroups.exact.length, 1)
    assert.equal(exact.comparison.state, 'COMPARABLE')
    assert.equal(exact.queryEvidence.perOrderCostLookup, false)
    const exported = await authority.exportWorkbook(user, { from: '2026-08-01', to: '2026-08-31' })
    const workbook = XLSX.read(exported.buffer, { type: 'buffer' })
    assert.deepEqual(workbook.SheetNames, ['经营利润汇总', '成本与完整性明细'])
    assert.equal(XLSX.utils.sheet_to_json(workbook.Sheets['经营利润汇总'])[0].利润状态, 'EXACT')
    ...
    const incompleteExport = await authority.exportWorkbook(user, { from: '2026-08-01', to: '2026-08-31' })
    const incompleteBook = XLSX.read(incompleteExport.buffer, { type: 'buffer' })
    assert.equal(XLSX.utils.sheet_to_json(incompleteBook.Sheets['经营利润汇总'], { defval: '' })[0]['经营利润/预估利润'], '')
    scope.days = days.map((date) => ({ storeKey: 'cost-store', storeName: '成本门店', date, authority: 'MANUAL_DAILY_ENTRY' }))
    const manual = await authority.report(user, { from: '2026-08-01', to: '2026-08-31' })
    assert.equal(manual.state, 'INCOMPLETE'); assert.ok(manual.completenessCodes.includes('INCOMPLETE_COGS')); assert.equal(manual.totals.cogsCents, null)
  } finally {
