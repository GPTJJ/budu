...
test.beforeEach(async ({ page }) => {
  ...
  // 选择角色 = 员工
  await dialog.locator('select').first().selectOption('staff')
...
test('勾选朝外店后绑定下拉包含马婧欣（档案门店 chaowai）', async ({ page }) => {
  ...
  await dialog.locator('button', { hasText: '北京朝外店' }).click()
  await page.waitForTimeout(500)
  const opts = await dialog.locator('select').nth(1).locator('option').allInnerTexts()
