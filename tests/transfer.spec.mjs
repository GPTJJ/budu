...
test('调拨详情使用单一内容滚动容器，边界 overscroll 不移动 header 或背景', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 720 })
  await page.goto('/tests/transfer-harness.html?records=1')
  await page.evaluate(() => window.scrollTo(0, document.documentElement.scrollHeight))
  const record = page.locator('[data-transfer-record-id="tr-pending"]')
  await record.scrollIntoViewIfNeeded()
  const backgroundY = await page.evaluate(() => window.scrollY)
  await record.click()
  await expect.poll(() => page.evaluate(() => document.body.style.position)).toBe('fixed')
  const dialog = page.getByRole('dialog', { name: '调拨详情' })
  const scroll = dialog.getByTestId('transfer-sheet-scroll')
  const header = dialog.getByTestId('transfer-sheet-header')
  ...
  expect(await page.evaluate(() => document.body.style.top)).toBe(`-${backgroundY}px`)
