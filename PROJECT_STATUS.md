...
当前 HEAD：`f6368ea`（V1.18），本地与 GitHub main 同步，腾讯云 /opt/budu 已部署该版本。

今日已完成并上线：
1. **V1.18 挑选货品多选**：申请调货/采购的挑选货品支持多选，产品、物料卡片可勾选多项后一次加入申请列表（统一填数量/备注，自动区分产品与物料，页签显示已选数量）；线上实测 15 项通过，未提交真实申请。
2. **V1.17 日历与工资全面体检**（多轮模拟测试通过）：
...
3. **此前版本累计**：发票开具+OCR、大单奖计入工资、调货/采购提醒扩展、版本更新推送、周末工资修复（V1.13-V1.16）
