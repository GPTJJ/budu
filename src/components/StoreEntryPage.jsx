...
function StaffPicker({ value, onChange }) {
  ...
  return (
    <div className="relative">
      ...
      >
        {value.length === 0 ? (
          <span className="text-slate-400">选择值班人员</span>
        ) : (
          value.map((n) => (
...
export default function StoreEntryPage({ onBack }) {
  ...
  const handleSave = () => {
    ...
    setVersion((v) => v + 1)
    setSavedTip(`已保存到本地 ✓ 值班人员：${staff.length > 0 ? staff.join('、') : '未选择'}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">门店业绩录入</h2>
      ...
      <div className="card p-5">
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          ...
          <Field label="值班人员" icon={Users}>
            <StaffPicker value={staff} onChange={setStaff} />
          </Field>

          <Field label="日期" icon={CalendarDays}>
        ...
        <p className={`mt-3 text-xs font-medium transition ${savedTip.startsWith('请') ? 'text-amber-500' : 'text-emerald-500'}`}>
          {savedTip || `当前录入：${monthLabel(month)} · ${storeInfo ? storeInfo.name : ''} · 选择日期自动归属对应月份`}
        </p>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 px-5 py-4">
          <h3 className="text-[15px] font-bold text-slate-800">业绩明细</h3>
          ...
          <div className="ml-auto flex gap-4 text-xs text-slate-400">
            <span>
              合计收入 <b className="text-slate-600">¥{formatMoney(summary.inc)}</b>
            </span>
            <span>
              合计订单 <b className="text-slate-600">{summary.ord.toLocaleString('zh-CN')}</b>
            </span>
            <span>
              客单价 <b className="text-slate-600">¥{summary.avgOrder.toFixed(2)}</b>
            </span>
          </div>
        </div>
          ...
          <table className="w-full min-w-[680px] text-left text-sm">
            <thead>
              <tr className="bg-slate-50/80 text-xs text-slate-400">
                ...
                <th className="px-4 py-3 font-semibold">值班人员</th>
                <th className="px-4 py-3 font-semibold">营业收入</th>
                <th className="px-4 py-3 font-semibold">订单数</th>
                <th className="px-4 py-3 font-semibold">客单价</th>
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.d} className="border-t border-slate-50 transition hover:bg-budu-50/40">
                    ...
                    <td className="px-4 py-3">
                      {r.local ? (
                        <span className="rounded-md bg-grape-50 px-1.5 py-0.5 text-[10px] font-bold text-grape-600">本地录入</span>
                      ) : (
                        <span className="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-400">报表</span>
                      )}
                    </td>
