...
export default function SweetCardPage({ user, onBack }) {
  ...
  const openDetail = async (id) => { setError(''); try { const result = await api(`/v2/sweet-cards/cards/${id}`); applyDetail(result.card) } catch (e) { setError(e.message) } }
  ...
  return <div className="min-h-full bg-slate-50 px-3 pb-28 pt-3 sm:px-6 sm:pt-6">
    <div className="mx-auto max-w-6xl">
      ...
      {loading ? <div className="mt-6 rounded-3xl bg-white p-10 text-center text-sm text-slate-400">正在读取甜意卡权威数据…</div> : config?.enabled !== true ? <div className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-8 text-center"><LockKeyhole className="mx-auto h-9 w-9 text-amber-500" /><h2 className="mt-3 font-black text-slate-800">Production capability 当前关闭</h2><p className="mt-2 text-sm text-slate-500">Candidate 已保留完整能力；开启前不会创建或核销任何真实价值。</p></div> : <>
        ...
        {tab === 'issue' && <form onSubmit={createBatch} className="mt-4 rounded-3xl bg-white p-5 shadow-sm sm:p-7">
          ...
          <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => setForm({ ...form, faceValueYuan: '500.00' })} className="rounded-full bg-budu-50 px-4 py-2 text-sm font-bold text-budu-600">¥500</button><button type="button" onClick={() => setForm({ ...form, faceValueYuan: '1000.00' })} className="rounded-full bg-budu-50 px-4 py-2 text-sm font-bold text-budu-600">¥1000</button><span className="self-center text-xs text-slate-400">或输入合法自定义额度</span></div>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label className="text-xs font-bold text-slate-500">面额（元）<input aria-label="面额（元）" inputMode="decimal" required value={form.faceValueYuan} onChange={(e) => setForm({ ...form, faceValueYuan: e.target.value })} placeholder="500.00" className={`${fieldClass} mt-1`} /></label>
            ...
            <label className="text-xs font-bold text-slate-500">通用备注（可选）<input value={form.recipientNote} onChange={(e) => setForm({ ...form, recipientNote: e.target.value })} className={`${fieldClass} mt-1`} /></label>
          </div>
    ...
    {detail && <div className="fixed inset-0 z-[120] flex items-end bg-slate-950/60 backdrop-blur-sm sm:items-center sm:justify-center sm:p-6" role="dialog" aria-modal="true" aria-label="甜意卡详情">
      <div className="max-h-[94dvh] w-full overflow-y-auto rounded-t-[30px] bg-white shadow-2xl sm:max-w-2xl sm:rounded-[30px]" style={{ paddingBottom: 'max(24px, env(safe-area-inset-bottom))' }}>
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-slate-100 bg-white/95 p-5 backdrop-blur"><div><p className="text-xs font-black tracking-widest text-budu-500">A LITTLE SWEETNESS.</p><h2 className="mt-1 text-xl font-black">{detail.publicCardNo}</h2></div><button onClick={() => setDetail(null)} className="rounded-xl bg-slate-100 px-3 py-2 text-sm font-bold text-slate-500">关闭</button></div>
          ...
          {detail.carrierType === 'ELECTRONIC' && <section className="rounded-3xl border border-rose-100 bg-rose-50/40 p-4" aria-label="电子卡发放">
            ...
            <form onSubmit={savePresentation} className="mt-5 rounded-2xl bg-white p-4" aria-label="编辑赠送信息">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="text-xs font-bold text-slate-500">赠送对象<input value={presentationForm.recipientLabel} onChange={(e) => setPresentationForm({ ...presentationForm, recipientLabel: e.target.value })} className={`${fieldClass} mt-1`} /></label>
                <label className="text-xs font-bold text-slate-500">赠送对象类型<input value={presentationForm.recipientType} onChange={(e) => setPresentationForm({ ...presentationForm, recipientType: e.target.value })} className={`${fieldClass} mt-1`} /></label>
                <label className="text-xs font-bold text-slate-500">公司（可选）<input value={presentationForm.recipientCompany} onChange={(e) => setPresentationForm({ ...presentationForm, recipientCompany: e.target.value })} className={`${fieldClass} mt-1`} /></label>
                <label className="text-xs font-bold text-slate-500">赠送场景（可选）<input value={presentationForm.giftingScenario} onChange={(e) => setPresentationForm({ ...presentationForm, giftingScenario: e.target.value })} className={`${fieldClass} mt-1`} /></label>
                <label className="text-xs font-bold text-slate-500 sm:col-span-2">祝福语 / campaign 文案<textarea rows="3" value={presentationForm.recipientNote} onChange={(e) => setPresentationForm({ ...presentationForm, recipientNote: e.target.value })} className={`${fieldClass} mt-1 resize-y`} /></label>
              </div>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {config?.claimPresentationEnabled && !detail.binding && detail.delivery?.claimStatus !== 'CLAIMED' && <button disabled={saving} onClick={() => generateClaimPresentation('ELECTRONIC')} className="min-h-12 rounded-xl bg-budu-500 px-4 font-bold text-white disabled:opacity-50">{detail.delivery?.claimCredentialStatus === 'ACTIVE' ? '重新生成领取凭证' : '生成电子卡'}</button>}
    ...
    {claimDelivery && <div className="fixed inset-0 z-[130] flex items-end bg-slate-950/70 backdrop-blur-sm sm:items-center sm:justify-center sm:p-6" role="dialog" aria-modal="true" aria-label="电子卡交付">
      <div className="max-h-[94dvh] w-full overflow-y-auto rounded-t-[30px] bg-white p-5 shadow-2xl sm:max-w-xl sm:rounded-[30px]">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2"><button onClick={downloadClaimAsset} className="min-h-11 rounded-xl border border-budu-200 font-bold text-budu-600">下载电子卡图片</button><button disabled={!claimDelivery.proofDelivery?.proof} onClick={copyClaimProof} className="min-h-11 rounded-xl bg-budu-500 font-bold text-white disabled:opacity-50">{claimDelivery.proofCopied ? '凭证已复制并从页面清除' : '复制独立领取凭证'}</button></div>
