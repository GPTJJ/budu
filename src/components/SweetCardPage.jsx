...
export default function SweetCardPage({ onBack }) {
  ...
  const [reportPurpose, setReportPurpose] = useState('COMMERCIAL')
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const purposeQuery = `?businessPurpose=${reportPurpose}`
      const [overview, batches, cards, rules, usage, audit] = await Promise.all([
        api(`/v2/sweet-cards/overview${purposeQuery}`), api(`/v2/sweet-cards/batches${purposeQuery}`), api(`/v2/sweet-cards/cards${purposeQuery}`), api('/v2/sweet-cards/rules'), api(`/v2/sweet-cards/usage${purposeQuery}`), api('/v2/sweet-cards/audit'),
      ])
  ...
  return <div className="min-h-full bg-slate-50 px-3 pb-28 pt-3 sm:px-6 sm:pt-6">
    <div className="mx-auto max-w-6xl">
      ...
      {loading ? <div className="mt-6 rounded-3xl bg-white p-10 text-center text-sm text-slate-400">正在读取甜意卡权威数据…</div> : config?.enabled !== true ? <div className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-8 text-center"><LockKeyhole className="mx-auto h-9 w-9 text-amber-500" /><h2 className="mt-3 font-black text-slate-800">Production capability 当前关闭</h2><p className="mt-2 text-sm text-slate-500">Candidate 已保留完整能力；开启前不会创建或核销任何真实价值。</p></div> : <>
        ...
        {['overview', 'batches', 'cards', 'usage'].includes(tab) && <div className="mt-2 flex gap-2 overflow-x-auto" aria-label="甜意卡批次用途视图">{[['COMMERCIAL', '商业运营'], ['ACCEPTANCE_TEST', '验收 / 测试']].map(([key, label]) => <button key={key} type="button" onClick={() => setReportPurpose(key)} className={`min-h-10 shrink-0 rounded-xl px-4 text-xs font-bold ${reportPurpose === key ? 'bg-slate-800 text-white' : 'bg-white text-slate-500 shadow-sm'}`}>{label}</button>)}</div>}
        ...
        {tab === 'batches' && <section className="mt-4 space-y-3">{data.batches.map((batch) => <article key={batch.id} className="rounded-3xl bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start gap-3"><div className="min-w-0 flex-1"><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-500">{batch.businessPurpose === 'COMMERCIAL' ? '商业运营' : '验收 / 测试'}</span><h3 className="mt-2 font-black text-slate-900">{batch.name}</h3><p className="mt-1 text-xs text-slate-400">{batch.cardCount} 张 · {formatCents(batch.faceValueCents)} · {batch.carrierType} · {batch.bindingMode}</p></div><a href={`/api/v2/sweet-cards/batches/${batch.id}/export`} className="inline-flex items-center gap-1 rounded-xl border border-budu-200 px-3 py-2 text-xs font-bold text-budu-600"><Download className="h-4 w-4" />QR 包</a></div><div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs"><span className="rounded-xl bg-slate-50 p-2">已激活 {batch.metrics.activated}</span><span className="rounded-xl bg-slate-50 p-2">已消费 {formatCents(batch.metrics.consumedCents)}</span><span className="rounded-xl bg-slate-50 p-2">余额 {formatCents(batch.metrics.balanceCents)}</span></div></article>)}</section>}
        {tab === 'cards' && <section className="mt-4">
          <div className="grid gap-2 rounded-3xl bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-4">
            ...
            <select aria-label="按状态筛选" value={cardFilter.status} onChange={(e) => setCardFilter({ ...cardFilter, status: e.target.value })} className={fieldClass}><option value="">全部状态</option>{['CREATED', 'ACTIVE', 'FROZEN', 'LOST', 'EXHAUSTED', 'EXPIRED', 'VOID'].map((status) => <option key={status}>{status}</option>)}</select>
          ...
          <div className="mt-3 grid gap-3 lg:grid-cols-2">{filteredCards.map((card) => <article key={card.id} className="rounded-3xl bg-white p-5 shadow-sm"><div className="flex justify-between gap-3"><div><h3 className="font-black text-slate-900">{card.publicCardNo}</h3><p className="mt-1 text-xs text-slate-400">{card.carrierType} · {card.bindingMode} · {card.status}</p></div><p className="text-xl font-black text-budu-600">{formatCents(card.balanceCents)}</p></div>{card.recipientLabel && <p className="mt-3 text-sm text-slate-600">赠予：{card.recipientLabel}</p>}<div className="mt-4 flex flex-wrap gap-2"><button onClick={() => openDetail(card.id)} className="rounded-xl border border-budu-200 px-3 py-2 text-xs font-bold text-budu-600">详情 / Ledger</button>{card.status === 'CREATED' && <button disabled={saving} onClick={() => transition(card.id, 'activate')} className="rounded-xl bg-budu-500 px-3 py-2 text-xs font-bold text-white">激活</button>}{card.status === 'ACTIVE' && <button disabled={saving} onClick={() => transition(card.id, 'freeze')} className="rounded-xl border px-3 py-2 text-xs font-bold text-slate-600">冻结</button>}{card.status === 'FROZEN' && <button disabled={saving} onClick={() => transition(card.id, 'unfreeze')} className="rounded-xl border px-3 py-2 text-xs font-bold text-slate-600">解冻</button>}{!['VOID', 'EXHAUSTED', 'EXPIRED'].includes(card.status) && <button disabled={saving} onClick={() => transition(card.id, 'void')} className="rounded-xl border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600">作废</button>}</div></article>)}</div>
        ...
        {tab === 'issue' && <form onSubmit={createBatch} className="mt-4 rounded-3xl bg-white p-5 shadow-sm sm:p-7">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label className="text-xs font-bold text-slate-500">载体<select value={form.carrierType} onChange={(e) => setForm({ ...form, carrierType: e.target.value })} className={`${fieldClass} mt-1`}><option value="PHYSICAL">实体卡</option><option value="ELECTRONIC">电子卡</option></select></label>
            <label className="text-xs font-bold text-slate-500">绑定模式<select value={form.bindingMode} onChange={(e) => setForm({ ...form, bindingMode: e.target.value })} className={`${fieldClass} mt-1`}><option value="NONE">NONE</option><option value="OPTIONAL">OPTIONAL</option><option value="REQUIRED">REQUIRED</option></select></label>
    ...
    {detail && <div className="fixed inset-0 z-[120] flex items-end bg-slate-950/60 backdrop-blur-sm sm:items-center sm:justify-center sm:p-6" role="dialog" aria-modal="true" aria-label="甜意卡详情">
      <div className="max-h-[92dvh] w-full overflow-y-auto rounded-t-[30px] bg-white p-5 shadow-2xl sm:max-w-2xl sm:rounded-[30px]" style={{ paddingBottom: 'max(24px, env(safe-area-inset-bottom))' }}>
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 text-sm">
          ...
          <p className="rounded-xl bg-slate-50 p-3">状态<br/><strong>{detail.status}</strong></p><p className="rounded-xl bg-slate-50 p-3">载体 / 绑定模式<br/><strong>{detail.carrierType} / {detail.bindingMode}</strong></p>
