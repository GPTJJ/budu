...
import {
  ...
  FileSpreadsheet, Landmark, ListFilter, PackageSearch, ReceiptText, Search, ShoppingBag,
...
import {
  coverageText, formatReportBps, formatReportCents, formatReportInteger, localReportTime,
...
function CompositionCard({ title, note, data, onCoverage }) {
  ...
  return (
    <section className="rounded-3xl border border-white bg-white p-4 shadow-card sm:p-5">
      <div className="flex items-start justify-between gap-3">
        <div><h3 className="font-bold text-slate-900">{title}</h3><p className="mt-1 text-xs leading-5 text-slate-400">{note}</p></div>
...
export default function ReportCenterPage({ currentUser, onBack, onNavigate }) {
  ...
  const [tab, setTab] = useState(canSales ? 'summary' : 'profit')
  ...
  }, [canSales, coreTab, tab, range.from, range.to, store, orderSource, settlementType, productSearch, productSort, page, reloadKey])
