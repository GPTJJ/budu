...
const profitStateText = { EXACT: '经营利润', ESTIMATED: '预估经营利润', INCOMPLETE: '经营利润' }
const profitStateBadge = { EXACT: '精确', ESTIMATED: '含预估成本', INCOMPLETE: '暂不可计算' }
const rentModeText = { FIXED: '固定月租', PERCENT: '营业抽成', FIXED_PLUS_PERCENT: '固定 + 抽成', MAX_FIXED_PERCENT: '固定与抽成取高' }
...
function OperatingProfitPanel({ currentUser, range, store, allowedStores, comparisonMode, period }) {
  ...
  useEffect(() => {
    if (!canView) return
    const params = new URLSearchParams({ from: range.from, to: range.to, compare: comparisonMode, period }); if (selectedStore) params.set('store', selectedStore)
  ...
  }, [canView, range.from, range.to, selectedStore, month, comparisonMode, period, reload])
  const download = async () => {
    const params = new URLSearchParams({ from: range.from, to: range.to, compare: comparisonMode, period }); if (selectedStore) params.set('store', selectedStore)
    setBusy(true); setError('')
    try {
      const response = await fetch(`/api/v2/report-center/operating-costs/export?${params}`, { credentials: 'same-origin' })
      if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.error || '经营利润导出失败') }
      const blob = await response.blob(); const url = URL.createObjectURL(blob); const anchor = document.createElement('a')
      anchor.href = url; anchor.download = `budu经营利润_${range.from}_${range.to}.xlsx`; anchor.click(); URL.revokeObjectURL(url)
  ...
  const margin = data?.profitMarginBps ?? data?.estimatedProfitMarginBps
  const selectedDetail = selectedStore ? data?.stores?.find((row) => row.storeKey === selectedStore) : null
  return <div className="space-y-4" data-testid="operating-profit-panel">
    <section className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold text-slate-400">管理口径 · 不等同于财务会计利润</p><h3 className="mt-1 text-lg font-black text-slate-900">{profitStateText[data?.state] || '经营利润'}</h3><p className="mt-2 text-3xl font-black text-slate-900">{profit == null ? '—' : formatReportCents(profit)}</p>{margin != null && <p className="mt-1 text-xs font-bold text-slate-400">{data?.state === 'ESTIMATED' ? '预估经营利润率' : '经营利润率'} {formatReportBps(margin)}</p>}</div><div className="flex items-center gap-2"><button type="button" onClick={download} disabled={busy || !data} className="inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 text-[11px] font-bold text-slate-600 disabled:opacity-40"><FileSpreadsheet className="h-3.5 w-3.5" />导出</button>{data?.state && <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${data.state === 'EXACT' ? 'bg-emerald-50 text-emerald-700' : data.state === 'ESTIMATED' ? 'bg-sky-50 text-sky-700' : 'bg-amber-50 text-amber-700'}`}>{profitStateBadge[data.state]}</span>}</div></div>
      ...
      <div className="mt-4 rounded-2xl border border-slate-100 p-3"><p className="text-xs font-black text-slate-700">数据完整性</p><div className="mt-2 grid grid-cols-2 gap-2 text-[11px] sm:grid-cols-3">{[['revenueCents','营业收入','PARTIAL_SOURCE_COVERAGE'],['cogsCents','商品成本','INCOMPLETE_COGS'],['laborCents','人工','INCOMPLETE_LABOR'],['rentCents','房租','INCOMPLETE_RENT'],['utilityCents','水电','INCOMPLETE_UTILITY'],['otherCents','其他费用','INCOMPLETE_OTHER']].map(([key,label,code]) => { const missing = data?.totals?.[key] == null; const estimated = (data?.estimateCodes || []).some((item) => key === 'utilityCents' ? item === 'ESTIMATED_UTILITY' : item === 'ESTIMATED_CURRENT_PERIOD'); return <div key={key} className={`rounded-xl px-2.5 py-2 font-bold ${missing ? 'bg-rose-50 text-rose-700' : estimated ? 'bg-sky-50 text-sky-700' : 'bg-emerald-50 text-emerald-700'}`}>{missing ? '✕' : estimated ? '△' : '✓'} {label}{missing && <span className="sr-only"> {code}</span>}</div> })}</div></div>
      ...
      {data?.state === 'INCOMPLETE' && <p className="mt-3 text-xs font-bold text-amber-700">已知成本仍按事实展示；关键成本缺失时不会猜算经营利润。</p>}
      {data?.comparison && <div className="mt-3 rounded-xl bg-slate-50 px-3 py-2.5 text-xs text-slate-600">{data.comparison.state === 'COMPARABLE' ? <><strong>{comparisonMode === 'year' ? '同比' : '环比'}：</strong>{data.comparison.changeBps == null ? '基期为 0，无法计算百分比' : formatComparisonBps(data.comparison.changeBps)}</> : <><strong>{comparisonMode === 'year' ? '同比' : '环比'}：</strong>— 数据口径不可比（仅比较两个 EXACT 且门店覆盖一致的期间）</>}</div>}
      {(data?.warningCodes || []).length > 0 && <p className="mt-3 text-xs text-rose-600">检测到 Expense 中可能存在房租/水电/人工类文本，请人工核对重复风险；系统不会自动删除或去重。</p>}
    </section>
    {(data?.stores || []).length > 1 && <section className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm sm:p-5"><h3 className="text-sm font-black text-slate-800">门店利润状态</h3><p className="mt-1 text-xs text-slate-400">仅 EXACT 门店参与利润排名；预估与不完整门店分区展示。</p>{[['exact','精确利润排名'],['estimated','预估'],['incomplete','数据不完整']].map(([key,label]) => (data?.storeGroups?.[key] || []).length > 0 && <div key={key} className="mt-4"><p className="text-[11px] font-bold text-slate-400">{label}</p><div className="mt-2 space-y-2">{data.storeGroups[key].map((row,index) => <div key={row.storeKey} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2.5 text-xs"><span className="truncate font-bold text-slate-700">{key === 'exact' ? `${index + 1}. ` : ''}{row.storeName}</span><span className="shrink-0 font-black text-slate-800">{key === 'incomplete' ? '—' : formatReportCents(row.exactOperatingProfitCents ?? row.estimatedOperatingProfitCents)}</span></div>)}</div></div>)}</section>}
    {selectedDetail && <section className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm sm:p-5"><h3 className="text-sm font-black text-slate-800">成本来源与明细</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">{Object.entries(selectedDetail.components || {}).map(([key, component]) => { const labels = { revenue:'营业收入',cogs:'商品销售成本',labor:'人工成本',rent:'房租',utility:'水电',other:'其他经营费用' }; return <div key={key} className="rounded-2xl bg-slate-50 p-3"><div className="flex items-start justify-between gap-2"><p className="text-xs font-black text-slate-700">{labels[key]}</p><span className="text-[10px] font-bold text-slate-400">{component.status}</span></div><p className="mt-2 text-base font-black text-slate-800">{formatReportCents(component.amountCents)}</p><p className="mt-1 text-[10px] text-slate-400">{component.source}</p>{key === 'labor' && <div className="mt-2 text-[10px] leading-5 text-slate-500">工资 {formatReportCents(component.breakdown?.wagesCents)} · 公司社保 {formatReportCents(component.breakdown?.socialSecurityCents)} · 公积金 {formatReportCents(component.breakdown?.providentFundCents)} · 其他 {formatReportCents(component.breakdown?.otherCents)}{component.confirmedZero && '（已确认 0）'}</div>}{key === 'rent' && component.months?.[0] && <p className="mt-2 text-[10px] text-slate-500">{rentModeText[component.months[0].mode] || '未配置'}{component.months[0].percentageBasis && ` · ${component.months[0].percentageBasis === 'GROSS_SALES' ? '营业额' : '营业收入'}为抽成基数`}</p>}{key === 'utility' && component.months?.[0] && <p className="mt-2 text-[10px] text-slate-500">预计 {formatReportCents(component.months[0].estimatedCents)}{component.months[0].actualCents != null ? ` · 实际 ${formatReportCents(component.months[0].actualCents)} · 差异 ${formatReportCents(component.months[0].differenceCents)}` : ' · 尚无实际值'}</p>}</div> })}</div></section>}
...
function DashboardProfitCard({ profit, onOpen }) {
  const unavailable = !profit?.available || profit.state === 'INCOMPLETE'
  const value = unavailable ? null : profit.valueCents
  return <button type="button" onClick={onOpen} className="min-w-0 rounded-2xl border border-white bg-white p-3.5 text-left shadow-sm sm:p-5" data-testid="dashboard-profit"><div className="grid h-9 w-9 place-items-center rounded-xl bg-slate-100 text-slate-500"><Landmark className="h-4 w-4" /></div><p className="mt-3 text-lg font-black tabular-nums text-slate-900">{formatReportCents(value)}</p><p className="mt-1 text-xs font-semibold text-slate-400">{profit?.label || '经营利润'}</p><p className={`mt-2 text-[10px] font-bold ${profit?.state === 'INCOMPLETE' ? 'text-amber-600' : 'text-slate-400'}`}>{!profit?.available ? '需要经营成本与人工成本权限' : profit.state === 'EXACT' ? '精确成本权威' : profit.state === 'ESTIMATED' ? '含预估成本' : completenessText[profit.reasonCode] || '成本数据不完整'}</p></button>
}
...
function DashboardReport({ data, loading, error, reload, storeMap, topSort, onTopSort, onCoverage, onTab }) {
  ...
  return <div className="space-y-4 sm:space-y-5">
    ...
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">{dashboardMetricDefinitions.map(([key, label, Icon, money, target]) => <DashboardMetricCard key={key} metricKey={key} label={label} Icon={Icon} money={money} metric={data?.metrics?.[key]} comparison={data?.comparisons?.[key]} onCoverage={onCoverage} onOpen={() => onTab(target)} />)}<DashboardProfitCard profit={data?.profit} onOpen={() => onTab('profit')} /></section>
...
export default function ReportCenterPage({ currentUser, onBack, onNavigate }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-5" data-testid="report-center-page">
      ...
      {tab === 'profit' && <OperatingProfitPanel currentUser={currentUser} range={range} store={store} allowedStores={allowedStores} comparisonMode={comparisonMode} period={preset} />}
