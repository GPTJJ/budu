...
const PAY_NOTE = '基础工资=基础时薪×工时；提成=提成时薪×工时；大单奖=订单金额×5%；当日工资=基础工资+提成+大单奖；1人值班按门店标准工时，2人及以上各8h；节假日/调休按2026年规则计算；未录入日期计0。'
...
export function createEmployeePayWorkbook({ employeeName, periodLabel, dayRows, totals }) {
  const header = ['日期', '值班门店', '营业额(元)', '订单', '工时(h)', '基础工资(元)', '业绩提成(元)', '大单奖(元)', '当日工资(元)']
  ...
  sheet['!merges'] = [
    { s: { r: 0, c: 0 }, e: { r: 0, c: 8 } },
    { s: { r: totalRowIndex + 2, c: 1 }, e: { r: totalRowIndex + 2, c: 8 } },
  ]
  sheet['!autofilter'] = { ref: `A${headerRow + 1}:I${totalRowIndex + 1}` }
  applyNumberFormat(sheet, tableRange, [2, 5, 6, 7, 8], '¥#,##0.00')
